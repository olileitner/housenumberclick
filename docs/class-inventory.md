...
| `HouseNumberClickPlugin` | `HouseNumberClickPlugin.java` | Yes | Plugin entry point that wires the menu action and performs one-time toolbar migration via centralized plugin preferences. |
...
| `HouseNumberClickPreferences` | `HouseNumberClickPreferences.java` | No | Centralized plugin preference access using the shared housenumberclick.* namespace. |
